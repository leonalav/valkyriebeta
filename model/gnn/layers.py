...
class GraphConvolution(nn.Module):
    """
    Simple Graph Convolutional layer for use in the hybrid RWKV-Transformer architecture.
    Optimized for sparse graph operations to handle long sequences efficiently.
    """
    ...
    def forward(
        ...
        Args:
            x: Node features of shape [batch_size, num_nodes, in_features]
            edge_indices: Edge indices of shape [2, num_edges]
        ...
        # Apply linear transformation to node features
        x_transformed = torch.matmul(x, self.weight)  # [batch_size, num_nodes, out_features]
        ...
        # Default edge weights to 1.0 if not provided
        if edge_weights is None:
            edge_weights = torch.ones(edge_indices.size(1), device=x.device)
        
        # Message passing (sparse implementation for memory efficiency)
        row, col = edge_indices
        for i in range(edge_indices.size(1)):
            batch_idx, src_idx = row[i]
            _, dst_idx = col[i]
            
            # Only process valid indices (within batch and node limits)
            if (batch_idx < batch_size and 
                src_idx < num_nodes and 
                dst_idx < num_nodes):
                
                weight = edge_weights[i] if edge_weights is not None else 1.0
                output[batch_idx, dst_idx] += weight * x_transformed[batch_idx, src_idx]
...
class GraphAttention(nn.Module):
    """
    Graph Attention Network (GAT) layer optimized for the hybrid RWKV-Transformer model.
    Uses multi-head attention for better expressivity and supports edge features.
    """
    ...
    def __init__(
        ...
        # If we're not concatenating heads, ensure out_features is divisible by num_heads
        if not concat_heads:
            assert out_features % num_heads == 0, "out_features must be divisible by num_heads if concat_heads is False"
        ...
        # Output projection for multi-head aggregation
        if concat_heads:
    ...
    def reset_parameters(self):
        ...
        if self.use_edge_features:
            nn.init.xavier_uniform_(self.edge_proj.weight, gain=gain)
    ...
    def forward(
        ...
        # Linear projection to get features for each attention head
        h = self.W(x)  # [batch_size, num_nodes, num_heads * head_dim]
        ...
        # Compute attention using src and dst projections for each edge
        src_h = h.index_select(1, src_indices[:, 1])  # [batch_size, num_edges, num_heads, head_dim]
        dst_h = h.index_select(1, dst_indices[:, 1])  # [batch_size, num_edges, num_heads, head_dim]
        
        # Compute attention scores
        src_attn = (src_h * self.a_src.view(1, 1, self.num_heads, self.head_dim)).sum(dim=-1)
        dst_attn = (dst_h * self.a_dst.view(1, 1, self.num_heads, self.head_dim)).sum(dim=-1)
        
        # Combine attention scores
        attn_scores = src_attn + dst_attn  # [batch_size, num_edges, num_heads]
        
        # Incorporate edge features if provided
        if self.use_edge_features and edge_features is not None:
            edge_attn = self.edge_proj(edge_features).view(-1, self.num_heads)
            attn_scores = attn_scores + edge_attn.view(1, -1, self.num_heads)
        
        # Apply LeakyReLU
        attn_scores = F.leaky_relu(attn_scores, negative_slope=self.alpha)
        
        # Apply softmax to normalize attention weights for each node
        # This is more complex with batched sparse graphs, so we do it per node
        normalized_attn = torch.zeros_like(attn_scores)
        
        # For each node in each batch, gather its outgoing edges and normalize those attention scores
        for b in range(batch_size):
            for node in range(num_nodes):
                # Find edges where this node is the source
                mask = (src_indices[:, 0] == b) & (src_indices[:, 1] == node)
                if mask.sum() > 0:
                    edge_indices_for_node = torch.where(mask)[0]
                    scores_for_node = attn_scores[b, edge_indices_for_node]
                    
                    # Apply softmax for this node's edges
                    normalized_scores = F.softmax(scores_for_node, dim=0)
                    
                    # Store normalized scores
                    normalized_attn[b, edge_indices_for_node] = normalized_scores
        
        # Apply dropout to attention weights
        normalized_attn = self.dropout(normalized_attn)
        
        # Apply attention weights to neighbor features
        output = torch.zeros_like(h)
        
        # Aggregate messages using attention weights
        for e in range(edge_indices.size(1)):
            b_src, n_src = src_indices[e]
            b_dst, n_dst = dst_indices[e]
            
            # Only process valid indices
            if (b_src < batch_size and b_dst < batch_size and 
                n_src < num_nodes and n_dst < num_nodes and
                b_src == b_dst):  # Only aggregate within same batch
                
                # Get attention weights for this edge
                attn_weights = normalized_attn[b_src, e]  # [num_heads]
                
                # Get source node features
                source_features = h[b_src, n_src]  # [num_heads, head_dim]
                
                # Weight source features by attention and aggregate at destination
                output[b_dst, n_dst] += attn_weights.unsqueeze(-1) * source_features
