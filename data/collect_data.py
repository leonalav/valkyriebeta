...
def load_olympiad_bench():
    ...
    # Check if solution is ever longer than 1 and handle accordingly
    solution_lengths = [len(item['solution']) for item in ds]
    max_solution_length = max(solution_lengths)
    
    # Function to properly format questions with context
    def format_with_context(example):
        # Add context to question if available
        question_text = example["question"]
        if 'context' in example and example['context'].strip():
            question_text = f"Context: {example['context']}\n\nQuestion: {question_text}"
        
        # Handle solutions with multiple parts
        solution_text = example["solution"][0] if len(example["solution"]) > 0 else ""
        if len(example["solution"]) > 1:
            # Concatenate multiple solution parts
        ...
        return {
            ...
            "cot_type": "math", 
            "source_type": "Hothan/OlympiadBench/" + example['question_type'] + "/" + example['subject'], 
            "metadata": str(example)
        }
    
    ds = ds.map(format_with_context)
    ds = ds.remove_columns([c for c in ds.column_names if c not in DS_COLUMNS])
    return ds
...
def select_examples_omni_math(clean_examples, n_examples):
    # Define the criteria for bad samples
    BAD_OMNIMATH_SAMPLES = [
        # Samples with malformed or incorrect solutions
        lambda x: len(x.get("solution", "")) < 10,  # Too short solutions
        lambda x: "I don't know" in x.get("solution", ""),  # Uncertain solutions
        lambda x: "cannot be solved" in x.get("solution", ""),  # Unsolvable problems
        
        # Samples with problematic content
        lambda x: "error" in x.get("solution", "").lower() and "margin of error" not in x.get("solution", "").lower(),
        
        # Samples with placeholder or incomplete content
        lambda x: "[placeholder]" in x.get("solution", "").lower(),
        lambda x: "to be determined" in x.get("solution", "").lower(),
        
        # Specific metadata criteria
        lambda x: eval(x.get("metadata", "{}")).get("quality_score", 1.0) < 0.7 if "metadata" in x else False,
    ]
    
    # Filter out bad samples
    filtered_examples = []
    for ex in clean_examples:
        # Skip example if any of the bad sample criteria match
        is_bad = False
        for bad_criterion in BAD_OMNIMATH_SAMPLES:
            try:
                if bad_criterion(ex):
                    is_bad = True
                    break
            except:
                # If evaluation fails, consider it problematic
                is_bad = True
                break
    ...
    # Continue with the existing selection logic
    random.shuffle(filtered_examples)
    all_subdomains = list()
    all_difficulties = list()
    # get all subdomains and difficulties
    for ex in tqdm(filtered_examples, desc="Selecting examples"):
...
if __name__ == "__main__":
    ...
    # Update paths to save locally
    output_dir = "/root/nanogpt/datasets/processed"
    ...
    for ds_name, (load_fn, selection_fn, n_samples) in DS_TO_SELECTION.items():
        ...
        ds_path = os.path.join(output_dir, f"{ds_name}.parquet")
        ds.to_parquet(ds_path)
        print(f"Saved {ds_name} to {ds_path}")
    ...
    # Save final combined dataset locally
    combined_path = os.path.join(output_dir, "s50K.parquet")
    ds.to_parquet(combined_path)
